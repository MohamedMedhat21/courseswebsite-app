...
export class Utils {
  ...
  static warningMessage = new Subject<string>();

  static formatDate(date: Date) {
    const creationDate = new Date(date);
    const days = Math.ceil(
      (this.currDate.getTime() - creationDate.getTime()) / (1000 * 3600 * 24)
    );
